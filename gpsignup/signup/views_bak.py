...
def results(request, event_id):    
    ...
    for slot in slot_list:
        date_dict[slot.slot_date] = 1
        time_str = str(slot.slot_start_time)[:-3]
...
def view(request, event_id):
    if request.method == 'POST':
        ...
        want_reminder = request.POST['want_reminder']
        ...
        if created:
            person.phone = phone
            person.person_code = uuid.uuid4()
            person.want_reminder = want_reminder
...
def manage(request):
    if request.method == 'POST':
        ...
        event_list = Event.objects.all().order_by('event_start_date', 'event_start_time')    
